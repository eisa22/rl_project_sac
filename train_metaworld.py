...
def make_mt1_env(task_name):
    """Creates single-task (MT1 style) MetaWorld env for evaluation."""
...
def load_mt10_envs():
    """Creates all 10 training envs for MT10."""
    mt10 = metaworld.MT10()
    envs = {}
    tasks = []
...
def evaluate_single(task_name, agent, episodes=5):
    ...
    return np.mean(returns), success_count / episodes
...
def train_mt10(total_steps=2_000_000, start_steps=3000, batch_size=512):
    ...
    # Replay Buffer w/ task one-hot
    replay_buffer = ReplayBuffer(obs_dim, act_dim, task_dim=num_tasks, size=int(1e6))

    # SAC Agent (paper parameters)
    agent = SACAgent(
        obs_dim=obs_dim + num_tasks,
        act_dim=act_dim,
        act_limit=act_limit,
        hidden_actor=(256, 256),            # paper small actor
        hidden_critic=(2048, 2048, 2048),   # paper scaled critic
        gamma=0.99,
        lr=3e-4,
        tau=0.005,
    )
    ...
    for t in range(1, total_steps + 1):
        ...
        # episode done → switch to new random task
        ...
        # ------------------------------------------------------------
        # EVALUATION (like paper ~ every 50k steps)
        # ------------------------------------------------------------
        if t % 50_000 == 0:
            print(f"\n📈 Eval @ step {t}")
