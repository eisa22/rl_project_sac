"""
MTMH-SAC Evaluation Script for MT3 with Simulator Visualization

Evaluates trained MTMH-SAC models on MT3 Meta-World tasks (reach, push, pick-place).
...
from mtmh_sac import MultiHeadActor

# MT3 Task definitions (3 tasks only)
TASK_NAMES = [
    'reach-v3',
    'push-v3',
    'pick-place-v3'
...
def load_model(model_path, device='cuda'):
    ...
    # MT3 default architecture (from train_mt3_mtmh.py)
    trunk_hidden_actor = config.get('trunk_hidden_actor', (256, 256))
    ...
    act_limit = 1.0
    num_tasks = 3  # MT3 has only 3 tasks!
    ...
    print(f"  Num tasks: {num_tasks} (MT3)")
...
def main():
    parser = argparse.ArgumentParser(description="MTMH-SAC MT3 Evaluation with Visualization")
    parser.add_argument("--model_path", type=str, default="models_mtmh/mt3/checkpoint_step3000000.pt",
                        help="Path to model checkpoint")
    parser.add_argument("--task", type=str, default="reach-v3",
                        help="Task to evaluate (reach-v3, push-v3, pick-place-v3, or 'all')")
    ...
    if args.task.lower() == 'all':
        ...
        if args.task not in TASK_NAMES:
            ...
            print(f"Available MT3 tasks: {TASK_NAMES}")
            return
    ...
    if len(all_results) > 1:
        print(f"\n{'='*60}")
        print("FINAL SUMMARY (MT3 Tasks)")
        print(f"{'='*60}")
        
        total_success = np.mean([r['success_rate'] for r in all_results])
        total_reward = np.mean([r['mean_reward'] for r in all_results])
        
        print(f"\n{'Task':<20} {'Success Rate':<15} {'Mean Reward':<15}")
        print("-" * 50)
        for r in all_results:
            print(f"{r['task']:<20} {r['success_rate']*100:>10.1f}% {r['mean_reward']:>15.2f}")
        print("-" * 50)
        print(f"{'AVERAGE':<20} {total_success*100:>10.1f}% {total_reward:>15.2f}")
