...
class MetaWorldCurriculumEnv(gym.Env):
    ...
    def _get_task_sampling_probs(self, available_tasks):
        """
        If pick-place-v3 is active:
          - pick-place-v3: 80%
          - remaining tasks: share 20% uniformly
        Otherwise:
          - uniform sampling
        """
        names = [t.env_name for t in available_tasks]

        if "pick-place-v3" not in names:
            # Uniform sampling
            return np.ones(len(available_tasks)) / len(available_tasks)

        probs = np.zeros(len(available_tasks), dtype=np.float32)

        pick_idx = names.index("pick-place-v3")
        probs[pick_idx] = 0.8

        remaining = len(available_tasks) - 1
        if remaining > 0:
            for i in range(len(probs)):
                if i != pick_idx:
                    probs[i] = 0.2 / remaining
    ...
    def _sample_task(self):
        if self.fixed_task_name and self.fixed_task_name in self.task_envs:
            ...
            probs = self._get_task_sampling_probs(available)
